...
#### See Forman et al. (2008), Sebastiani (2015), González et al. (2017)
...
adjust.prev <- function(unadj,table){
  ...
  if (sum(diag(table) == 0)>0) {stop(
    ...
    if(class(try(solve(rates))) != "matrix"){stop(
      ...
      if (sum(est.vector>0) != length(est.vector))
      {
        ...
        cat(underZero,"class was estimated as negative - corrected as zero \n \n")
